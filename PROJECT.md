...
**Releases are automated.** A monthly CI job (the 28th, roughly a week after ESPHome's
third-Wednesday release, so we pick up its early patch releases too) rebuilds every device
against the current ESPHome release and publishes the next patch version, skipping the
release when neither ESPHome nor the device configs have changed (compared by hashing
`common/`, `devices/` and `firmware/`, so CI-only commits don't reflash the fleet) and
opening an issue when a rebuild breaks. This is what keeps the
...
- **Productized devices get unique API keys too** - Firmware ships keyless
  (`api: encryption:` with no key); Home Assistant provisions a per-device key on
  first connection, stored in flash
- **Local OTA port has a published password** - Not a secret (it's in this repo);
  blocks drive-by scanners, documented in the README security model
