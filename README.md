...
- **Automatically, monthly.** On the 28th of each month CI rebuilds every device against the
  ...
  the first round of patch releases, rather than on day-one `.0`. If neither ESPHome nor the
  device configs (`common/`, `devices/`, `firmware/`) have changed since the last release, the
  run stops early rather than reflashing your devices with identical firmware — CI and docs
  changes alone don't trigger one. If a rebuild fails, CI opens an issue.
